...
public class FeignClient {
     ...
     *
     * @param smartWebClient
     * @param url
     * @param body
     * @param resultType
     * @param unSuccessHandle
     * @param <T>
     * @return
     */
    public <T> Mono<T> restMonoPostJson(WebClient smartWebClient, String url,
                                        HashMap<String, Object> body, Class<T> resultType, Function<ClientResponse,
            Mono<? extends Throwable>> unSuccessHandle) {
        return smartWebClient
                .method(HttpMethod.POST) // post get put ……
                .uri(url) // 这里的params是拼接在url中的，可以用@PathVariable接收的参数
                .accept(MediaType.APPLICATION_JSON) // Accept 一般是响应的content-type
                .contentType(MediaType.APPLICATION_JSON) // content-type
                // 这里可以放入BodyInserters.from……多种方法，按需使用
                .body(BodyInserters.fromValue(body)) // 构造的参数可以用@RequestParam、@RequestBody接收
                .retrieve() // 获取响应体
    ...
    public <T> Mono<T> restMono(WebClient smartWebClient,
        ...
        return smartWebClient
                ...
                .accept(respMediaType) // Accept 一般是响应的content-type
                .contentType(reqMediaType) // content-type
                // 这里可以放入BodyInserters.from……多种方法，按需使用
                .body(BodyInserters.fromValue(body)) // 构造的参数可以用@RequestParam、@RequestBody接收
                .retrieve() // 发送请求
                // 第一个参数判断状态，为调用方法的条件，第二个传入lambda表达式，返回Mono<异常>
